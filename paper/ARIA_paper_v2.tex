...
\begin{table}[h]
\centering
\caption{Component ablation on Split-MNIST (to be filled after running \texttt{scripts/ablation.py}).}
\label{tab:ablation}
\begin{tabular}{lcc}
\toprule
...
\begin{itemize}
  \item Evaluated only on Split-MNIST; Split-CIFAR-10 and longer benchmarks are in progress.
  \item Morphogenesis did not activate on Split-MNIST; harder benchmarks are needed
    to validate MA's contribution.
  \item Fisher estimation is diagonal; Kronecker-factored approximations may improve estimates.
  \item DER++ used a small buffer (200 samples); a larger buffer would be a fairer comparison.
\end{itemize}
